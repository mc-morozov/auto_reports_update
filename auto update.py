...
class Report():
    ...
    def permission_444(self):
        """Установка прав только на чтение документа. Применяется к сохраняемому файлу"""
        try:
            os.chmod(self.save_report, 0o444)
        except:
            print(f'Ошибка установки прав: {self.save_report}')

    def permission_777(self):
        """Установка прав на чтение и изменение документа. Применяется к открываемому файлу."""
        try:
            os.chmod(self.open_report, 0o777)
        except:
            print(f'Ошибка установки прав: {self.open_report}')
...
with closing(psycopg2.connect(dbname='analytics', user='analytics', password='5sCfQMCKt8e0F4', host='252-pg01', port='5432')) as conn:
